...
# Part of the hack for force-loading sounding text,
# This makes sure at least something was passed, if not then die!
if len(sys.argv) < 3:
	quit('You must pass a filename containing sounding data and filename for the PID text file, exiting.')
dataFilename = sys.argv[1]
pidFilename = sys.argv[2]
model_title_name = sys.argv[3]
configFile = sys.argv[4]
if model_title_name == 'WRF':
	model_title_name = 'NAM'

if model_title_name == 'NAMNST':
	model_init_time = dataFilename[49:51]
	model_f_time = dataFilename[59:62]
elif model_title_name == 'HRRR':
    model_init_time = dataFilename[49:51]
    model_f_time = dataFilename[57:60]
else:
	model_init_time = dataFilename[49:51]
	model_f_time = dataFilename[56:59]
...
class Picker(QWidget):
    ...
    def skewApp(self, filename=None):
        ...
        if filename is not None:
            model = model_init_time + '00 UTC '  + model_title_name + ' F' + model_f_time
...
class Main(QMainWindow):
    ...
    HOME_DIR = os.path.join(os.path.expanduser("~"), ".sharppy")
    cfg_file_name = os.path.join(HOME_DIR,'sharppy.'+configFile+'.ini')
    ...
    def __initUI(self):
        ...
        self.picker.skewApp(filename=dataFilename)
